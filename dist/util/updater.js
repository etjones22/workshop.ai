...
export async function checkForUpdates(repoDir) {
...
async function resolveRemoteRef(git, branch) {
    if (branch && branch !== "HEAD") {
        const ref = `origin/${branch}`;
        const exists = await git(["show-ref", "--verify", "--quiet", `refs/remotes/${ref}`]).then(() => true, () => false);
        if (exists) {
            return ref;
        }
    }
