...
export async function fsList(root, inputPath = ".") {
...
export async function fsRead(root, inputPath) {
    const rootReal = await ensureWorkspaceRoot(root);
    const resolved = await resolveSandboxPath(rootReal, inputPath);
    const content = await fs.readFile(resolved.absolutePath, "utf8");
    return { path: resolved.relativePath, content };
}
export async function fsWrite(root, inputPath, content, overwrite = false) {
    const rootReal = await ensureWorkspaceRoot(root);
    const resolved = await resolveSandboxPath(rootReal, inputPath);
...
export async function fsApplyPatch(root, patch) {
    const rootReal = await ensureWorkspaceRoot(root);
...
async function applySimplePatch(rootReal, patch) {
...
async function applyUnifiedDiff(rootReal, patchText) {
...
function parseSimplePatch(patch) {
