...
export interface VoskSetupOptions {
  baseDir: string;
  onStatus?: (message: string) => void;
  progress?: ProgressReporter;
}

export async function ensureVoskRuntime(options: VoskSetupOptions): Promise<VoskRuntime> {
...
export async function transcribeWithVosk(runtime: VoskRuntime, wavPath: string): Promise<string> {
  ...
  const { stdout } = await execFileAsync(
    runtime.pythonPath,
    [scriptPath, "--model", runtime.modelPath, "--input", wavPath],
    {
      windowsHide: true,
...
async function ensureVoskModel(options: VoskSetupOptions): Promise<string> {
...
async function downloadWithProgress(url: string, dest: string, progress?: ProgressReporter): Promise<void> {
...
async function fetchRemoteMeta(url: string): Promise<{ etag?: string; lastModified?: string } | null> {
  try {
...
function isMetaUpToDate(
  localMeta: { url?: string; etag?: string; lastModified?: string },
  remoteMeta: { etag?: string; lastModified?: string } | null,
  modelUrl: string
): boolean {
...
async function readLocalMeta(metaPath: string): Promise<{ url?: string; etag?: string; lastModified?: string; modelDir?: string } | null> {
  try {
    const content = await fs.readFile(metaPath, "utf8");
    return JSON.parse(content) as { url?: string; etag?: string; lastModified?: string; modelDir?: string };
  } catch {
    return null;
  }
}

async function checkVoskInstalled(pythonPath: string): Promise<boolean> {
  try {
...
async function canExecute(cmd: string, args: string[]): Promise<boolean> {
  try {
