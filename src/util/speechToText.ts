...
import { ensureVoskRuntime, transcribeWithVosk, type VoskRuntime } from "./vosk.js";
...
export function createPushToTalk(options: PushToTalkOptions): PushToTalk {
  ...
  let listener: ((e: { name?: string; state?: string }, down: Record<string, boolean>) => void) | null = null;
  let recordingProcess: ReturnType<typeof spawn> | null = null;
  let recordingPath: string | null = null;
  let isBusy = false;
  let recorderCmd: RecorderCommand | null = null;
  let voskRuntime: VoskRuntime | null = null;
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (!recorderCmd) {
        options.onError?.(
          "Recorder not found. Install sox and ensure it is on PATH, or set SOX_BIN/REC_BIN."
        );
        return;
      }
  ...
  const transcribe = async (wavPath: string) => {
    ...
    try {
      ...
      if (!whisper) {
        options.onError?.(
          "No STT engine available. Configure Vosk (default) or set WHISPER_CPP_BIN and WHISPER_CPP_MODEL."
        );
        return;
      }
...
async function resolveRecorderCommand(): Promise<RecorderCommand | null> {
...
async function resolveWhisperCommand(): Promise<{ bin: string; model: string } | null> {
...
async function safeCleanup(paths: string[]): Promise<void> {
  await Promise.all(
